...
app = Flask(__name__)
app.secret_key = 'inventory-secret-key-2024'
...
# ========== ORDERS API (FIXED - Auto-generate unique ID if duplicate) ==========
...
def advanced_orders_api():
    try:
        ...
        # Get current location from request or use default
        ...
        # ========== FIXED: Handle order ID with duplicate check ==========
        final_order_id = None
        ...
        if not user_order_id or user_order_id.strip() == '':
            ...
            if existing:
                # ID exists, auto-generate a new unique ID
        ...
        # Process each item and create order
        created_items = []
        for item in items:
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # Add new columns to users table if not exists (for existing databases)
        try:
